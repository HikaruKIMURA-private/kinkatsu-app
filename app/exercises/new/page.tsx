  ...
  return (
    <div className="container mx-auto py-8 px-4">
      <Card className="mx-auto max-w-md">
        ...
        <CardContent>
          <form id={form.id} onSubmit={(e) => {
            // #region agent log
            fetch('http://127.0.0.1:7242/ingest/8b4a1d01-fe61-4473-825d-4c2d3a3c271b',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({location:'app/exercises/new/page.tsx:55',message:'Form submit triggered',data:{},timestamp:Date.now(),sessionId:'debug-session',runId:'run1',hypothesisId:'C'})}).catch(()=>{});
            // #endregion
            form.onSubmit(e);
          }} action={formAction} className="space-y-4">
            {/* エラーメッセージ */}
            {state?.status === "error" && (
            ...
            <div className="space-y-2">
              ...
              <div className="grid grid-cols-3 gap-2" role="group" aria-label="部位選択">
                  ...
                  return (
                    ...
                    >
                      <input
                        ...
                        onChange={(e) => {
                          if (e.target.checked) {
                            setSelectedBodyParts([...selectedBodyParts, option.value]);
                          } else {
                            setSelectedBodyParts(selectedBodyParts.filter((bp) => bp !== option.value));
                          }
                        }}
                        className="sr-only"
                      />
              ...
              {state?.status === "error" && state.issues && state.issues.some((issue) => issue.path.some((p) => p === "bodyParts")) && (
                <p className="text-sm text-destructive">
                  {state.issues.find((issue) => issue.path.some((p) => p === "bodyParts"))?.message || "部位を選択してください"}
                </p>
              )}
            </div>
            ...
            <div className="flex gap-2">
              <Button type="submit" className="flex-1" disabled={state?.status === "success"}>
